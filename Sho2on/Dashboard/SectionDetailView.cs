...
namespace HR_Application.Dashboard
{
    ...
    public class AttendanceRowViewModel
    {
        ...
        public string StatusText =>
            _att.IsAbsence ? LocalizationManager.Translate("غائب") :
            _att.Late.HasValue ? LocalizationManager.Translate("متأخر") :
                                     LocalizationManager.Translate("حاضر");
    ...
    public partial class SectionDetailView : UserControl
    {
        ...
        private async Task LoadDataAsync()
        {
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في تحميل بيانات القطاع: {ex.Message}", LocalizationManager.Translate("خطأ"),
        ...
        private async Task LoadCharts()
        {
            ...
            var pieSeries = new SeriesCollection
            {
                new PieSeries
                {
                    Title = LocalizationManager.Translate("حاضر"), Values = new ChartValues<int> { present },
                    DataLabels = true,
                    LabelPoint = p => $"{p.Y} ({p.Participation:P0})"
                },
                new PieSeries
                {
                    Title = LocalizationManager.Translate("غائب"), Values = new ChartValues<int> { absent },
                    DataLabels = true,
                    LabelPoint = p => $"{p.Y} ({p.Participation:P0})"
                },
                new PieSeries
                {
                    Title = LocalizationManager.Translate("متأخر"), Values = new ChartValues<int> { late },
                    DataLabels = true,
                    LabelPoint = p => $"{p.Y} ({p.Participation:P0})"
                }
            };
            ...
            SectionDeptBar.Series = new SeriesCollection
            {
                new ColumnSeries
                {
                    Title = LocalizationManager.Translate("الموظفين"), Values = deptCounts, DataLabels = true
                }
            };
        }
