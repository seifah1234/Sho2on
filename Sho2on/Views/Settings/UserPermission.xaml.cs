    ...
    public partial class UserPermission : Window
    {
        ...
        private async Task LoadRolesAsync()
        {
            try
            {
                ...
                // إضافة عنصر LocalizationManager.Translate("لا شيء")
                _roles.Add(new Role { RoleID = 0, RoleName = LocalizationManager.Translate("لا شيء") });
        ...
        private async Task SaveDataAsync()
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("يرجى اختيار مستخدم واحد", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                if (roleComboBox.SelectedItem is not Role selectedRole)
                {
                    LocalizationManager.ShowMessage("يرجى اختيار دور", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                LocalizationManager.ShowMessage("تم تحديث صلاحيات المستخدم بنجاح", LocalizationManager.Translate("نجاح"), MessageBoxButton.OK, MessageBoxImage.Information);
        ...
        private async Task ManageUserRolesAsync(User user, int roleId)
        {
            ...
            // إضافة الدور الجديد إذا لم يكن LocalizationManager.Translate("لا شيء")
            if (roleId > 0)
            {
