    ...
    public partial class AddLate : Window
    {
        ...
        private async Task SaveData(int moneyType)
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("يرجى اختيار وقت البداية والنهاية", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                if (string.IsNullOrWhiteSpace(valueTextBox.Text))
                {
                    LocalizationManager.ShowMessage("يرجى إدخال القيمة", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                if (!decimal.TryParse(valueTextBox.Text, out decimal value))
                {
                    LocalizationManager.ShowMessage("يرجى إدخال قيمة رقمية صحيحة", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                {
                    LocalizationManager.ShowMessage("يرجى إدخال وقت صالح", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                if (fromTime >= toTime)
                {
                    LocalizationManager.ShowMessage("وقت البداية يجب أن يكون قبل وقت النهاية", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                LocalizationManager.ShowMessage("تم حفظ البيانات بنجاح", LocalizationManager.Translate("نجاح"), MessageBoxButton.OK, MessageBoxImage.Information);
                await LoadData();
                ClearForm(moneyType);
            }
            catch (Exception ex)
        ...
        private async Task EditData(int moneyType)
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("يرجى اختيار عنصر للتعديل", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                LocalizationManager.ShowMessage("تم تعديل البيانات بنجاح", LocalizationManager.Translate("نجاح"), MessageBoxButton.OK, MessageBoxImage.Information);
                await LoadData();
                ClearForm(moneyType);
            }
            catch (Exception ex)
        ...
        private async Task DeleteData(int moneyType)
        {
            try
            {
                ...
                if (selectedItem == null)
                {
                    LocalizationManager.ShowMessage("يرجى اختيار عنصر للحذف", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                if (result == MessageBoxResult.Yes)
                {
                    ...
                    LocalizationManager.ShowMessage("تم حذف البيانات بنجاح", LocalizationManager.Translate("نجاح"), MessageBoxButton.OK, MessageBoxImage.Information);
                    await LoadData();
                    ClearForm(moneyType);
                }
            }
