...
namespace HR_Application.Views.Settings
{
    public partial class NetworkSettingsWindow : Window
    {
        ...
        private async void LoadCurrentSettings()
        {
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في تحميل الإعدادات: {ex.Message}", LocalizationManager.Translate("خطأ"),
        ...
        private async void testNetworkBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                LocalizationManager.ShowMessage("يرجى إدخال مسار صحيح", LocalizationManager.Translate("تحذير"),
            ...
            try
            {
                ...
                networkStatusText.Text = LocalizationManager.Translate("جاري اختبار الاتصال...");

                bool isAccessible = await Task.Run(() => TestNetworkPath(path));

                if (isAccessible)
                {
                    networkStatusText.Text = LocalizationManager.Translate("? الاتصال ناجح - المسار متاح");
                    ...
                    if (canWrite)
                    {
                        networkStatusText.Text += LocalizationManager.Translate(" - صلاحيات الكتابة متاحة");
                    }
                    else
                    {
                        networkStatusText.Text += LocalizationManager.Translate(" - تحذير: لا توجد صلاحيات كتابة");
                        networkStatusText.Foreground = System.Windows.Media.Brushes.Orange;
                    }
                }
                else
                {
                    networkStatusText.Text = LocalizationManager.Translate("? الاتصال فاشل - المسار غير متاح");
        ...
        private void ShowConnectionSuggestions(string failedPath)
        {
            ...
            string suggestions = LocalizationManager.Translate("اقتراحات لحل المشكلة:\n\n");
            ...
            if (failedPath.StartsWith(@"\\"))
            {
                ...
                {
                    suggestions += LocalizationManager.Translate("   غير قادر على اكتشاف IPs الشبكة\n");
                }
            }

            suggestions += LocalizationManager.Translate("\n6. يمكنك استخدام المسار المحلي مؤقتاً");
        ...
        private void btnLocalIP_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في الحصول على IP المحلي: {ex.Message}", LocalizationManager.Translate("خطأ"),
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private string GetLocalIPAddress()
        {
            ...
            throw new Exception(LocalizationManager.Translate("لا يوجد عنوان IP محلي"));
        }
        ...
        private async void permissionTestBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (canWrite)
                {
                    ShowTestResult(LocalizationManager.Translate("? صلاحيات الكتابة متاحة"), true);
                }
                else
                {
                    ShowTestResult(LocalizationManager.Translate("? لا توجد صلاحيات كتابة على المجلد"), false);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task UpdateStatistics()
        {
            try
            {
                ...
                connectionStatusText.Text = isConnected ? LocalizationManager.Translate("متصل ?") : LocalizationManager.Translate("غير متصل ?");
                ...
                {
                    freeSpaceText.Text = LocalizationManager.Translate("غير متاح");
                }
            }
            catch
            {
                currentPathText.Text = LocalizationManager.Translate("غير قادر على تحديد المسار");
                connectionStatusText.Text = LocalizationManager.Translate("خطأ في الاتصال");
                freeSpaceText.Text = LocalizationManager.Translate("غير متاح");
            }
        }
        ...
        private void initFoldersBtn_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                LocalizationManager.ShowMessage("تم إنشاء المجلدات الهيكلية بنجاح", LocalizationManager.Translate("نجاح"),
                    MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                LocalizationManager.ShowMessage($"خطأ في إنشاء المجلدات: {ex.Message}", LocalizationManager.Translate("خطأ"),
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        {
            var result = LocalizationManager.ShowMessage("هل تريد نقل الملفات الحالية إلى المسار الجديد؟",
                LocalizationManager.Translate("تأكيد النقل"), MessageBoxButton.YesNo, MessageBoxImage.Question);
            ...
            try
            {
                ...
                LocalizationManager.ShowMessage("تم نقل الملفات بنجاح", LocalizationManager.Translate("نجاح"),
                    MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                LocalizationManager.ShowMessage($"خطأ في نقل الملفات: {ex.Message}", LocalizationManager.Translate("خطأ"),
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
        ...
        private async void saveBtn_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("المسار غير متاح. يرجى التحقق من الاتصال.", LocalizationManager.Translate("خطأ"),
                        MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                LocalizationManager.ShowMessage("تم حفظ الإعدادات بنجاح", LocalizationManager.Translate("نجاح"),
                    MessageBoxButton.OK, MessageBoxImage.Information);
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في حفظ الإعدادات: {ex.Message}", LocalizationManager.Translate("خطأ"),
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
