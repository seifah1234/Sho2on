    ...
    public partial class AddJob : Window
    {
        ...
        private async void save_Btn(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("أدخل اسم الوظيفة", "خطأ", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                MessageBox.Show("تم إضافة الوظيفة", "", MessageBoxButton.OK, MessageBoxImage.Information);
                LoadData();
            }
            catch
            {
                MessageBox.Show("حدث خطأ أثناء الحفظ", "خطأ", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async void delete_Btn(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("لم يتم اختيار الوظيفة", "خطأ", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            ...
            try
            {
                ...
                await _context.SaveChangesAsync();
                MessageBox.Show("تم حذف الوظيفة", "", MessageBoxButton.OK, MessageBoxImage.Information);
                LoadData();
            }
            catch
            {
                MessageBox.Show("حدث خطأ أثناء الحذف", "خطأ", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async void edit_Btn(object sender, RoutedEventArgs e)
        {
            if (list.SelectedItem is not JobTitle selectedJob)
            {
                MessageBox.Show("لم تختار أي وظيفة", "خطأ", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
            ...
            try
            {
                ...
                MessageBox.Show("تم تعديل الوظيفة", "", MessageBoxButton.OK, MessageBoxImage.Information);
                LoadData();
            }
            catch
            {
                MessageBox.Show("حدث خطأ أثناء التعديل", "خطأ", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
