    ...
    public partial class AddOffical : Window
    {
        ...
        private async void save_Btn(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("أدخل المسمى", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                var user = await _context.Users.FirstOrDefaultAsync(u => u.Code == code_box.Text);
                if (user == null)
                {
                    LocalizationManager.ShowMessage("الرجاء اختيار الموظف");
                    return;
                }
        ...
        private async void delete_Btn(object sender, EventArgs e)
        {
            ...
            {
                LocalizationManager.ShowMessage("لم يتم اختيار المسؤول", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
        ...
        private async void edit_Btn(object sender, EventArgs e)
        {
            if (_selectedOffical == null)
            {
                LocalizationManager.ShowMessage("لم تختار أي مسؤول", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
