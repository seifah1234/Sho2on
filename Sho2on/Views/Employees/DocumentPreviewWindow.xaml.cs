...
namespace HR_Application.Views
{
    public partial class DocumentPreviewWindow : Window
    {
        ...
        private async void LoadDocument()
        {
            try
            {
                ...
                if (_isEmployeeDocument)
                {
                    ...
                    {
                        ShowErrorAndClose(LocalizationManager.Translate("الوثيقة غير موجودة"));
                        return;
                    }
                ...
                {
                    ShowFallbackMessage(LocalizationManager.Translate("الملف غير موجود على السيرفر"));
                    return;
                }
        ...
        private async System.Threading.Tasks.Task LoadPdfDocumentAsync()
        {
            try
            {
                using (var document = PdfDocument.Open(_filePath))
                {
                    ...
                    if (_pdfPageCount == 0)
                    {
                        ShowFallbackMessage(LocalizationManager.Translate("ملف PDF فارغ"));
                        return;
                    }
        ...
        private async System.Threading.Tasks.Task RenderPdfPageWithSkiaAsync(PdfDocument document, int pageNumber)
        {
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في عرض الصفحة: {ex.Message}");
            }
            finally
            {
                loadingIndicator.Visibility = Visibility.Collapsed;
        ...
        private void LoadImageDocument()
        {
            try
            {
                ...
                txtPageInfo.Text = LocalizationManager.Translate("صورة");
            }
            catch (Exception ex)
            {
        ...
        private void PrintPdfDocument(PrintDialog printDialog)
        {
            try
            {
                ...
                LocalizationManager.ShowMessage($"تم إرسال الوثيقة '{documentTitle}' للطباعة", LocalizationManager.Translate("نجاح"),
                    MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                LocalizationManager.ShowMessage($"خطأ في طباعة PDF: {ex.Message}", LocalizationManager.Translate("خطأ"),
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void PrintWithDefaultApp()
        {
            try
            {
                ...
                LocalizationManager.ShowMessage("تم فتح الملف بالبرنامج الافتراضي للطباعة",
                    LocalizationManager.Translate("معلومات"), MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                LocalizationManager.ShowMessage($"لا يمكن طباعة هذا النوع من الملفات مباشرة: {ex.Message}",
        ...
        private void downloadBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في التحميل: {ex.Message}", LocalizationManager.Translate("خطأ"),
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        {
            LocalizationManager.ShowMessage(message, LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
            Close();
        }
