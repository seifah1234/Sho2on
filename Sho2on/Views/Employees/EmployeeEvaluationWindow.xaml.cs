...
namespace HR_Application.Views
{
    public partial class EmployeeEvaluationWindow : Window
    {
        ...
        private async Task LoadEvaluationData()
        {
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في تحميل بيانات التقييم: {ex.Message}", LocalizationManager.Translate("خطأ"),
        ...
        {
            AddCriteria(_administrativeCriteria, txtNewAdministrativeCriteria, LocalizationManager.Translate("إداري"));
        }
        ...
        {
            AddCriteria(_technicalCriteria, txtNewTechnicalCriteria, LocalizationManager.Translate("فني"));
        }
        ...
        private void AddCriteria(ObservableCollection<EvaluationCriteriaModel> criteriaList, TextBox textBox, string type)
        {
            ...
            {
                LocalizationManager.ShowMessage($"يرجى إدخال اسم المعيار {type}", LocalizationManager.Translate("تحذير"),
        ...
        private void CriteriaRadio_Checked(object sender, RoutedEventArgs e)
        {
            ...
            if (criteria != null)
            {
                criteria.IsSuccessful = radio.Content.ToString() == LocalizationManager.Translate("موفق");
                CalculateStatistics();
            }
        }
        ...
        private async void btnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                LocalizationManager.ShowMessage("يرجى إضافة معايير تقييم على الأقل", LocalizationManager.Translate("تحذير"),
                    MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            {
                LocalizationManager.ShowMessage("يرجى تحديد النتيجة النهائية", LocalizationManager.Translate("تحذير"),
                    MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            try
            {
                ...
                LocalizationManager.ShowMessage("تم حفظ التقييم بنجاح", LocalizationManager.Translate("نجاح"),
                    MessageBoxButton.OK, MessageBoxImage.Information);

            }
            catch (Exception ex)
            {
                LocalizationManager.ShowMessage($"خطأ في حفظ التقييم: {ex.Message}", LocalizationManager.Translate("خطأ"),
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async void txtEmployeeCodeSearch_KeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                ...
                if (!string.IsNullOrEmpty(code))
                {
                    ...
                    {
                        LocalizationManager.ShowMessage("لم يتم العثور على الموظف", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
            }
        }
        ...
        private void txtEmployeeNameSearch_KeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            if (e.Key == Key.Enter)
            {
                /*var employeeSearch = new EmployeeSelectionWindow(_context.Users.ToList(), false, LocalizationManager.Translate("اختر الموظف ليتم تقييمه"), txtEmployeeNameSearch.Text);
