...
namespace HR_Application.Views
{
    public partial class HolidayRequestWindow : Window
    {
        ...
        private async Task LoadManagers(int? jobTitleId = null)
        {
            try
            {
                using (var dbContext = new AppDbContext(App.ConnectionString))
                {
                    ...
                    {
                        btnSelectManager.Content = LocalizationManager.Translate("لا يوجد مديرين متاحين");
        ...
        private void btnSelectManager_Click(object sender, RoutedEventArgs e)
        {
            ...
            var managerSelectionWindow = new EmployeeSelectionWindow(_managers, true, LocalizationManager.Translate("اختر الموافق على الإجازة"));
        ...
        private void UpdateSelectedManagerDisplay(User manager)
        {
            ...
            txtSelectedManagerJobTitle.Text = manager.JobTitle?.Name ?? LocalizationManager.Translate("غير محدد");
            txtSelectedManagerDepartment.Text = manager.Department?.Name ?? LocalizationManager.Translate("غير محدد");

            panelSelectedManager.Visibility = Visibility.Visible;
            btnSelectManager.Content = LocalizationManager.Translate("تغيير المدير");
        }
        ...
        private async void UpdateUIForLeaveType(int leaveTypeId)
        {
            ...
            txtMaxConsecutiveDays.Text = leaveType.MaxConsecutiveDays?.ToString() ?? LocalizationManager.Translate("لا يوجد حد");
            txtRequiresApproval.Text = leaveType.RequiresApproval ? LocalizationManager.Translate("نعم") : LocalizationManager.Translate("لا");
        ...
        private void ClearManagerSelection()
        {
            ...
            btnSelectManager.Content = LocalizationManager.Translate("اختر الموافق على الإجازة");
        }
        ...
        private async void UpdateLeaveDuration(object sender, SelectionChangedEventArgs e)
        {
            if (startDate_picker.SelectedDate != null && endDate_picker.SelectedDate != null)
            {
                ...
                {
                    duration_box.Text = "0";
                    LocalizationManager.ShowMessage("تاريخ النهاية يجب أن يكون بعد تاريخ البداية");
                }
            }
        }
        ...
        private async void SearchEmployee()
        {
            try
            {
                ...
                var employeeSelectionWindow = new EmployeeSelectionWindow(allUsers, false, LocalizationManager.Translate("اختر الموظف لطلب الإجازة"), employeeCode_box.Text);
        ...
        private void DisplayEmployeeInfo(User employee)
        {
            ...
            txtEmployeeDepartment.Text = employee.Department?.Name ?? LocalizationManager.Translate("غير محدد");
            txtEmployeeBranch.Text = employee.Branch?.Name ?? LocalizationManager.Translate("غير محدد");
            ...
            {
                txtEmployeeWeekend.Text = LocalizationManager.Translate("غير محدد");
            }
        }
        ...
        private void ResetEmployeeInfo()
        {
            ...
            annualBalance_text.Text = LocalizationManager.Translate("0 يوم");
            remainingBalance_text.Text = LocalizationManager.Translate("0 يوم");
            usedBalance_text.Text = LocalizationManager.Translate("0 يوم");
        ...
        private async System.Threading.Tasks.Task UpdateLeaveBalanceDisplay(int userId, int leaveTypeId)
        {
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في حساب الرصيد: {ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async System.Threading.Tasks.Task CheckLeaveBalance()
        {
            ...
            {
                LocalizationManager.ShowMessage($"الرصيد المتبقي غير كافي. المتبقي: {balanceInfo.Remaining} يوم، المطلوب: {requestedDays} يوم",
            ...
            {
                LocalizationManager.ShowMessage($"الحد الأقصى للإجازة من هذا النوع هو {leaveType.MaxConsecutiveDays.Value} يوم متتالية",
                    LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
        ...
        private async System.Threading.Tasks.Task CheckDateConflicts(int requestedDays)
        {
            ...
            {
                var conflictMessage = LocalizationManager.Translate("هناك إجازات متعارضة في الفترة المحددة:\n");
                foreach (var leave in conflictingLeaves)
                {
                    conflictMessage += $"- من {leave.StartDate:yyyy/MM/dd} إلى {leave.EndDate:yyyy/MM/dd}\n";
                }

                LocalizationManager.ShowMessage(conflictMessage, LocalizationManager.Translate("تعارض في التواريخ"), MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        }
            ...
            if (ValidateForm())
            {
                try
                {
                    ...
                    string message = leaveRequest.Status == 2
                        ? LocalizationManager.Translate("تم تقديم طلب الإجازة بنجاح وتحديث سجلات الحضور")
                        : LocalizationManager.Translate("تم تقديم طلب الإجازة بنجاح وهو قيد انتظار الموافقة");

                    LocalizationManager.ShowMessage(message, LocalizationManager.Translate("نجاح"), MessageBoxButton.OK, MessageBoxImage.Information);
                    this.Close();
                }
                catch (Exception ex)
                {
                    LocalizationManager.ShowMessage($"خطأ في تقديم الطلب: {ex.InnerException?.Message ?? ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
        ...
        {
            var result = LocalizationManager.ShowMessage("هل تريد إلغاء طلب الإجازة؟", LocalizationManager.Translate("تأكيد الإلغاء"),
        ...
        private bool ValidateForm()
        {
            ...
            {
                LocalizationManager.ShowMessage("يرجى إدخال كود الموظف", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
            ...
            if (_employeeId == 0)
            {
                LocalizationManager.ShowMessage("يرجى البحث عن الموظف أولاً", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
            ...
            {
                LocalizationManager.ShowMessage("يرجى اختيار نوع الإجازة", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
            ...
            {
                LocalizationManager.ShowMessage("يرجى اختيار الموافق على الإجازة", LocalizationManager.Translate("خطأ"),
                    MessageBoxButton.OK, MessageBoxImage.Error);
            ...
            if (startDate_picker.SelectedDate == null || endDate_picker.SelectedDate == null)
            {
                LocalizationManager.ShowMessage("يرجى تحديد تاريخي بداية ونهاية الإجازة", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }

            if (startDate_picker.SelectedDate.Value < DateTime.Today)
            {
                LocalizationManager.ShowMessage("لا يمكن تقديم إجازة بتاريخ قديم", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
            ...
            {
                LocalizationManager.ShowMessage("يرجى إدخال مدة إجازة صحيحة", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
            ...
            {
                LocalizationManager.ShowMessage("يرجى كتابة سبب الإجازة", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                reason_box.Focus();
                return false;
            }
        ...
        private void btnManageBalance_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                LocalizationManager.ShowMessage("الرجاء البحث عن موظف أولاً", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        }
        ...
        private async void employeeCode_box_KeyDown(object sender, System.Windows.Input.KeyEventArgs e)
        {
            if (e.Key == System.Windows.Input.Key.Tab || e.Key == System.Windows.Input.Key.Enter)
            {
                ...
                if (!string.IsNullOrEmpty(code))
                {
                    ...
                    {
                        LocalizationManager.ShowMessage("لم يتم العثور ع الموظف", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
            }
        }
        ...
        private void searchReplace_btn_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                // فتح نافذة اختيار الموظف
                var employeeSelectionWindow = new EmployeeSelectionWindow(allUsers, false, LocalizationManager.Translate("اختر الموظف القائم عن العمل"), ReplaceEmployeeCode_box.Text);
