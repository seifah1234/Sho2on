    ...
    public partial class ManageLeaveBalanceWindow : Window, INotifyPropertyChanged
    {
        ...
        public async void btnSearch_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("·„ Ì „ «·⁄ÀÊ— ⁄·Ï „ÊŸ›Ì‰", "»ÕÀ", MessageBoxButton.OK, MessageBoxImage.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void LoadLeaveBalances()
        {
            ...
            {
                LocalizationManager.ShowMessage($"Œÿ√ ›Ì  Õ„Ì· —’Ìœ «·≈Ã«“« : {ex.Message}", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void UpdateEmployeeInfo()
        {
            if (SelectedUser != null)
            {
                txtEmployeeInfo.Text = $"{SelectedUser.FullName} - ﬂÊœ: {SelectedUser.Id}";
            ...
            {
                txtEmployeeInfo.Text = "·„ Ì „ «Œ Ì«— „ÊŸ›";
                txtEmployeeDetails.Text = "«·—Ã«¡ «·»ÕÀ ⁄‰ „ÊŸ›";
        ...
        private async void btnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (SelectedUser == null)
                {
                    LocalizationManager.ShowMessage("«·—Ã«¡ «Œ Ì«— „ÊŸ› √Ê·«", " Õ–Ì—", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                        ...
                        try
                        {
                            foreach (var balanceVM in LeaveBalances)
                            {
                                if (balanceVM.TotalBalance < balanceVM.UsedBalance)
                                {
                                    ...
                                    LocalizationManager.ShowMessage($"«·—’Ìœ «·ﬂ·Ì ··≈Ã«“… '{balanceVM.LeaveTypeName}' ·« Ì„ﬂ‰ √‰ ÌﬂÊ‰ √ﬁ· „‰ «·„” Œœ„ ({balanceVM.UsedBalance})",
                            ...
                            LocalizationManager.ShowMessage(" „ Õ›Ÿ —’Ìœ «·≈Ã«“«  »‰Ã«Õ", "‰Ã«Õ", MessageBoxButton.OK, MessageBoxImage.Information);
                        }
                        catch (Exception ex)
                        {
                            try
                            ...
                            LocalizationManager.ShowMessage($"Œÿ√ ›Ì «·Õ›Ÿ: {ex.InnerException}", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                            throw;
                        }
                    }
                });
        ...
        private bool ValidateLeaveBalances()
        {
            foreach (var balance in LeaveBalances)
            {
                ...
                {
                    LocalizationManager.ShowMessage($"ﬁÌ„… «·—’Ìœ ··≈Ã«“… '{balance.LeaveTypeName}' ÌÃ» √‰  ﬂÊ‰ ⁄œœ« ’ÕÌÕ« „ÊÃ»«",
                        "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                    return false;
                }
            }
        ...
        {
            if (LocalizationManager.ShowMessage("Â·  —Ìœ ≈⁄«œ…  ⁄ÌÌ‰ Ã„Ì⁄ «·√—’Ìœ ≈·Ï «·ﬁÌ„ «·«› —«÷Ì…ø",
                " √ﬂÌœ ≈⁄«œ… «· ⁄ÌÌ‰", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
            {
                LoadLeaveBalances();
            }
        }
