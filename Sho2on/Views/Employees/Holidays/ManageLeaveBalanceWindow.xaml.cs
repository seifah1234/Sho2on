    ...
    public partial class ManageLeaveBalanceWindow : Window, INotifyPropertyChanged
    {
        ...
        public async void btnSearch_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("لم يتم العثور على موظفين", LocalizationManager.Translate("بحث"), MessageBoxButton.OK, MessageBoxImage.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void LoadLeaveBalances()
        {
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في تحميل رصيد الإجازات: {ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void UpdateEmployeeInfo()
        {
            ...
            {
                txtEmployeeInfo.Text = LocalizationManager.Translate("لم يتم اختيار موظف");
                txtEmployeeDetails.Text = LocalizationManager.Translate("الرجاء البحث عن موظف");
        ...
        private async void btnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (SelectedUser == null)
                {
                    LocalizationManager.ShowMessage("الرجاء اختيار موظف أولاً", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                        ...
                        try
                        {
                            foreach (var balanceVM in LeaveBalances)
                            {
                                if (balanceVM.TotalBalance < balanceVM.UsedBalance)
                                {
                                    ...
                                    LocalizationManager.ShowMessage($"الرصيد الكلي للإجازة '{balanceVM.LeaveTypeName}' لا يمكن أن يكون أقل من المستخدم ({balanceVM.UsedBalance})",
                            ...
                            LocalizationManager.ShowMessage("تم حفظ رصيد الإجازات بنجاح", LocalizationManager.Translate("نجاح"), MessageBoxButton.OK, MessageBoxImage.Information);
                        }
                        catch (Exception ex)
                        {
                            try
                            ...
                            LocalizationManager.ShowMessage($"خطأ في الحفظ: {ex.InnerException}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                            throw;
                        }
                    }
                });
        ...
        private bool ValidateLeaveBalances()
        {
            foreach (var balance in LeaveBalances)
            {
                ...
                {
                    LocalizationManager.ShowMessage($"قيمة الرصيد للإجازة '{balance.LeaveTypeName}' يجب أن تكون عدداً صحيحاً موجباً",
                        LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                    return false;
                }
            }
        ...
        {
            if (LocalizationManager.ShowMessage("هل تريد إعادة تعيين جميع الأرصيد إلى القيم الافتراضية؟",
                LocalizationManager.Translate("تأكيد إعادة التعيين"), MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
            {
                LoadLeaveBalances();
            }
        }
