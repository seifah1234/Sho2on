...
namespace HR_Application.Views.Employees.Holidays
{
    public partial class LeaveTypesManagementWindow : Window
    {
        ...
        private async void LoadLeaveTypes()
        {
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في تحميل أنواع الإجازات: {ex.Message}", LocalizationManager.Translate("خطأ"),
        ...
        private async void btnDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (button != null && button.Tag is int leaveTypeId)
            {
                var result = LocalizationManager.ShowMessage("هل أنت متأكد من حذف هذا النوع من الإجازات؟",
                ...
                if (result == MessageBoxResult.Yes)
                {
                    try
                    {
                        ...
                        if (leaveType != null)
                        {
                            ...
                            if (isUsed)
                            {
                                LocalizationManager.ShowMessage("لا يمكن حذف هذا النوع لأنه مستخدم في طلبات إجازة",
                            ...
                            LoadLeaveTypes();
                            LocalizationManager.ShowMessage("تم حذف نوع الإجازة بنجاح", LocalizationManager.Translate("نجاح"),
                                MessageBoxButton.OK, MessageBoxImage.Information);
                        }
                    }
                    catch (Exception ex)
                    {
                        LocalizationManager.ShowMessage($"خطأ في الحذف: {ex.Message}", LocalizationManager.Translate("خطأ"),
                            MessageBoxButton.OK, MessageBoxImage.Error);
                    }
                }
            }
