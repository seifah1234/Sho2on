    ...
    public partial class LeaveBalanceReportWindow : Window, INotifyPropertyChanged
    {
        ...
        private void LoadDepartments()
        {
            try
            {
                ...
                cmbDepartment.Items.Add(new ComboBoxItem { Content = LocalizationManager.Translate("جميع الإدارات"), Tag = -1 });
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في تحميل الإدارات: {ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async void LoadReport()
        {
            try
            {
                ...
                foreach (var user in users)
                {
                    var employeeReport = new EmployeeLeaveBalanceReport
                    {
                        ...
                        DepartmentName = user.Department?.Name ?? LocalizationManager.Translate("غير معروف")
                    };
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في تحميل التقرير: {ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
...
private void ExportToExcel()
    {
        try
        {
            ...
            {
                Filter = LocalizationManager.Translate("ملفات Excel|*.xlsx"),
            ...
            if (saveFileDialog.ShowDialog() == true)
            {
                using (var workbook = new XLWorkbook())
                {
                    ...
                    var worksheet = workbook.Worksheets.Add(LocalizationManager.Translate("تقرير رصيد الإجازات"));
                    ...
                    titleRange.Merge();
                    titleRange.Value = LocalizationManager.Translate("تقرير رصيد الإجازات");
                    ...
                    // معلومات التقرير
                    worksheet.Cell(2, 1).Value = LocalizationManager.Translate("تاريخ التصدير:");
                    worksheet.Cell(2, 2).Value = DateTime.Now.ToString("yyyy/MM/dd HH:mm");
                    worksheet.Cell(3, 1).Value = LocalizationManager.Translate("عدد الموظفين:");
                    worksheet.Cell(3, 2).Value = ReportData.Count;
                    worksheet.Cell(4, 1).Value = LocalizationManager.Translate("أنواع الإجازات:");
                    ...
                    int currentRow = 6;
                    worksheet.Cell(currentRow, 1).Value = LocalizationManager.Translate("كود الموظف");
                    worksheet.Cell(currentRow, 2).Value = LocalizationManager.Translate("اسم الموظف");
                    worksheet.Cell(currentRow, 3).Value = LocalizationManager.Translate("الإدارة");
                    ...
                    foreach (var leaveType in leaveTypes)
                    {
                        ...
                        worksheet.Cell(currentRow + 1, colIndex).Value = LocalizationManager.Translate("المجموع");
                        worksheet.Cell(currentRow + 1, colIndex + 1).Value = LocalizationManager.Translate("المستخدم");
                        worksheet.Cell(currentRow + 1, colIndex + 2).Value = LocalizationManager.Translate("المتبقي");
                        worksheet.Cell(currentRow + 1, colIndex + 3).Value = LocalizationManager.Translate("النسبة %");
                    ...
                    LocalizationManager.ShowMessage($"تم تصدير التقرير بنجاح إلى:\n{saveFileDialog.FileName}",
                        LocalizationManager.Translate("نجاح"), MessageBoxButton.OK, MessageBoxImage.Information);

                    // فتح الملف بعد التصدير (اختياري)
                    if (LocalizationManager.ShowMessage("هل تريد فتح ملف Excel الآن؟", LocalizationManager.Translate("فتح الملف"),
        ...
        {
            LocalizationManager.ShowMessage($"خطأ في التصدير: {ex.Message}\n\n{ex.InnerException?.Message}",
    ...
    {
        var summarySheet = workbook.Worksheets.Add(LocalizationManager.Translate("ملخص"));

        // تنسيق النص من اليمين لليسار
        summarySheet.RightToLeft = true;

        // العنوان
        summarySheet.Cell(1, 1).Value = LocalizationManager.Translate("ملخص رصيد الإجازات");
        ...
        // رأس الجدول
        summarySheet.Cell(3, 1).Value = LocalizationManager.Translate("نوع الإجازة");
        summarySheet.Cell(3, 2).Value = LocalizationManager.Translate("إجمالي الرصيد");
        summarySheet.Cell(3, 3).Value = LocalizationManager.Translate("إجمالي المستخدم");
        summarySheet.Cell(3, 4).Value = LocalizationManager.Translate("متوسط النسبة %");
    ...
    private void AddStatisticsSheet(XLWorkbook workbook)
    {
        ...
        // العنوان
        statsSheet.Cell(1, 1).Value = LocalizationManager.Translate("إحصائيات التقرير");
        ...
        // إحصائيات عامة
        int row = 3;
        statsSheet.Cell(row, 1).Value = LocalizationManager.Translate("عدد الموظفين:");
        statsSheet.Cell(row, 2).Value = ReportData.Count;

        statsSheet.Cell(++row, 1).Value = LocalizationManager.Translate("أنواع الإجازات:");
        statsSheet.Cell(row, 2).Value = ReportData.FirstOrDefault()?.LeaveBalances.Count ?? 0;

        statsSheet.Cell(++row, 1).Value = LocalizationManager.Translate("تاريخ التصدير:");
        statsSheet.Cell(row, 2).Value = DateTime.Now.ToString("yyyy/MM/dd HH:mm");

        statsSheet.Cell(++row, 1).Value = LocalizationManager.Translate("عدد الموظفين بدون رصيد:");
        int zeroBalanceCount = ReportData.Count(e => e.LeaveBalances.All(l => l.Remaining == 0));
        statsSheet.Cell(row, 2).Value = zeroBalanceCount;

        statsSheet.Cell(++row, 1).Value = LocalizationManager.Translate("نسبة الموظفين بدون رصيد:");
    ...
    private void btnPrint_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    printDialog.PrintVisual(itemsReport, LocalizationManager.Translate("تقرير رصيد الإجازات"));
                }
            }
            catch (Exception ex)
            {
