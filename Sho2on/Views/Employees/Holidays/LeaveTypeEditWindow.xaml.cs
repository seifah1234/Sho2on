...
namespace HR_Application.Views.Employees.Holidays
{
    public partial class LeaveTypeEditWindow : Window
    {
        ...
        private void InitializeWindow()
        {
            if (!_isEditMode)
            {
                windowTitle.Text = LocalizationManager.Translate("نوع إجازة جديد");
            }
        }
        ...
        private async void LoadLeaveTypeData()
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("لم يتم العثور على نوع الإجازة", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                    this.Close();
                }
            }
            catch (Exception ex)
        ...
        private async void btnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (_isEditMode)
                {
                    ...
                    LocalizationManager.ShowMessage("تم تحديث نوع الإجازة بنجاح", LocalizationManager.Translate("نجاح"), MessageBoxButton.OK, MessageBoxImage.Information);
                }
                else
                {
                    ...
                    LocalizationManager.ShowMessage("تم إنشاء نوع الإجازة بنجاح", LocalizationManager.Translate("نجاح"), MessageBoxButton.OK, MessageBoxImage.Information);
                }
        ...
        private bool ValidateInput()
        {
            ...
            {
                LocalizationManager.ShowMessage("الرجاء إدخال اسم نوع الإجازة", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                txtName.Focus();
                return false;
            }
            ...
            {
                LocalizationManager.ShowMessage("الرجاء إدخال كود النوع", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                txtCode.Focus();
                return false;
            }
            ...
            {
                LocalizationManager.ShowMessage("الرجاء إدخال قيمة صحيحة للرصيد الافتراضي", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
            ...
            if (!string.IsNullOrWhiteSpace(txtMaxConsecutiveDays.Text))
            {
                if (!int.TryParse(txtMaxConsecutiveDays.Text, out int maxDays) || maxDays < 0)
                {
                    LocalizationManager.ShowMessage("الرجاء إدخال قيمة صحيحة للحد الأقصى للأيام", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
            ...
            if (!_isEditMode)
            {
                ...
                if (existingCode)
                {
                    LocalizationManager.ShowMessage("هذا الكود مستخدم مسبقاً، الرجاء اختيار كود آخر", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    txtCode.Focus();
                    return false;
                }
            }
