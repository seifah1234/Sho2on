...
/*
            ...
            logoCell.Blocks.Add(new Paragraph(new Run(LocalizationManager.Translate("شعار الشركة")))
            {
                FontSize = 10,
            ...
            titleParagraph.Inlines.Add(new Run(LocalizationManager.Translate("طلب إجازة"))
            {
                FontSize = 22,
        ...
        {
            // عنوان القسم
            Paragraph sectionTitle = new Paragraph(new Run(LocalizationManager.Translate("1. معلومات الموظف")))
            {
                FontSize = 14,
                FontWeight = FontWeights.Bold,
            ...
            // إضافة الصفوف
            string[,] employeeData = {
                { LocalizationManager.Translate("كود الموظف"), _user?.Id.ToString() ?? LocalizationManager.Translate("غير متوفر"), LocalizationManager.Translate("اسم الموظف"), _user?.FullName ?? LocalizationManager.Translate("غير متوفر") },
                { LocalizationManager.Translate("الإدارة"), _user?.Department?.Name ?? LocalizationManager.Translate("غير متوفر"), LocalizationManager.Translate("الفرع"), _user?.Branch?.Name ?? LocalizationManager.Translate("غير متوفر") },
                { LocalizationManager.Translate("تاريخ التعيين"), _user?.HireDate.ToString("yyyy/MM/dd") ?? LocalizationManager.Translate("غير متوفر"), LocalizationManager.Translate("الوظيفة"), _user?.JobTitle?.Name ?? LocalizationManager.Translate("غير متوفر") },
                { LocalizationManager.Translate("نظام العمل"), _user?.JobType?.Name ?? LocalizationManager.Translate("غير متوفر"), LocalizationManager.Translate("الوردية"), _user?.Shift?.Name ?? LocalizationManager.Translate("غير متوفر") }
            };
        ...
        {
            // عنوان القسم
            Paragraph sectionTitle = new Paragraph(new Run(LocalizationManager.Translate("2. معلومات الإجازة")))
            {
                FontSize = 14,
                TextAlignment = TextAlignment.Left,
                FontWeight = FontWeights.Bold,
            ...
            // إضافة الصفوف
            string[,] leaveData = {
                { LocalizationManager.Translate("نوع الإجازة"), _leaveType?.Name ?? LocalizationManager.Translate("غير متوفر"), LocalizationManager.Translate("رقم النوع"), _leaveType?.Code ?? LocalizationManager.Translate("غير متوفر") },
                { LocalizationManager.Translate("من تاريخ"), _leave.StartDate.ToString("yyyy/MM/dd"), LocalizationManager.Translate("إلى تاريخ"), _leave.EndDate.ToString("yyyy/MM/dd") },
                { LocalizationManager.Translate("المدة"), $"{_leave.Duration} يوم", LocalizationManager.Translate("تاريخ الطلب"), _leave.RequestDate.ToString("yyyy/MM/dd HH:mm") },
                { LocalizationManager.Translate("يتطلب موافقة"), _leaveType?.RequiresApproval == true ? LocalizationManager.Translate("نعم") : LocalizationManager.Translate("لا"),
                  LocalizationManager.Translate("يخصم من الرصيد"), _leaveType?.DeductFromBalance == true ? LocalizationManager.Translate("نعم") : LocalizationManager.Translate("لا") }
            };
            ...
            // قسم السبب
            Paragraph reasonTitle = new Paragraph(new Run(LocalizationManager.Translate("سبب الإجازة:")))
            {
                TextAlignment = TextAlignment.Left,
                FontSize = 12,
                FontWeight = FontWeights.Bold,
            ...
            Paragraph reasonParagraph = new Paragraph(new Run(_leave.Reason ?? LocalizationManager.Translate("لا يوجد سبب")))
            {
                FontSize = 11,
                TextAlignment = System.Windows.TextAlignment.Left,
                LineHeight = 20
        ...
        {
            // عنوان القسم
            Paragraph sectionTitle = new Paragraph(new Run(LocalizationManager.Translate("3. حالة الطلب")))
            {
                FontSize = 14,
                TextAlignment = TextAlignment.Left,
                FontWeight = FontWeights.Bold,
            ...
            statusLabelCell.Blocks.Add(new Paragraph(new Run(LocalizationManager.Translate("الحالة:")))
            {
                TextAlignment = TextAlignment.Left,
                FontWeight = FontWeights.SemiBold
            });
            ...
            {
                AddStatusDetailRow(statusGroup, LocalizationManager.Translate("تاريخ الموافقة/الرفض"),
                    _leave.ApprovalDate?.ToString("yyyy/MM/dd HH:mm") ?? LocalizationManager.Translate("غير متوفر"));

                AddStatusDetailRow(statusGroup, LocalizationManager.Translate("تمت الموافقة بواسطة"),
                    _leave.Approver?.FullName ?? LocalizationManager.Translate("غير متوفر"));

                if (_leave.Status == 3 && !string.IsNullOrEmpty(_leave.RejectionReason))
                {
                    AddStatusDetailRow(statusGroup, LocalizationManager.Translate("سبب الرفض"), _leave.RejectionReason);
                }
            }
            else if (_leave.Status == 4) // ملغى
            {
                AddStatusDetailRow(statusGroup, LocalizationManager.Translate("تاريخ الإلغاء"),
                    _leave.CancelledDate?.ToString("yyyy/MM/dd HH:mm") ?? LocalizationManager.Translate("غير متوفر"));

                AddStatusDetailRow(statusGroup, LocalizationManager.Translate("تم الإلغاء بواسطة"),
                    _leave.Canceller?.FullName ?? LocalizationManager.Translate("غير متوفر"));

                if (!string.IsNullOrEmpty(_leave.CancellationReason))
                {
                    AddStatusDetailRow(statusGroup, LocalizationManager.Translate("سبب الإلغاء"), _leave.CancellationReason);
                }
            }
        ...
        {
            // عنوان القسم
            Paragraph sectionTitle = new Paragraph(new Run(LocalizationManager.Translate("4. معلومات الرصيد")))
            {
                FontSize = 14,
                TextAlignment = TextAlignment.Left,
                FontWeight = FontWeights.Bold,
            ...
            // إضافة الصناديق
            AddBalanceBox(balanceGrid, 0, LocalizationManager.Translate("الرصيد الكلي"), _totalBalance.ToString(),
                Color.FromRgb(46, 125, 50), "E8F5E8");

            AddBalanceBox(balanceGrid, 1, LocalizationManager.Translate("المستخدم"), _usedBalance.ToString(),
                Color.FromRgb(198, 40, 40), "FFEBEE");

            AddBalanceBox(balanceGrid, 2, LocalizationManager.Translate("المتبقي"), _remainingBalance.ToString(),
        ...
        private void AddFooter(FlowDocument document)
        {
            ...
            // التوقيعات
            string[] signatures = {
                LocalizationManager.Translate("توقيع الموظف\n\n________________\nالتاريخ: ____/____/____"),
                LocalizationManager.Translate("توقيع رئيس القسم\n\n________________\nالتاريخ: ____/____/____"),
                LocalizationManager.Translate("توقيع مدير الموارد البشرية\n\n________________\nالتاريخ: ____/____/____")
            };
            ...
            // حقوق النشر
            Paragraph copyright = new Paragraph(new Run(LocalizationManager.Translate("© نظام إدارة الموارد البشرية - جميع الحقوق محفوظة")))
            {
                FontSize = 8,
                Foreground = Brushes.LightGray,
                TextAlignment = System.Windows.TextAlignment.Center,
        ...
        public void PrintToXps(string filePath)
        {
            try
            {
                ...
                LocalizationManager.ShowMessage($"تم حفظ الملف في: {filePath}", LocalizationManager.Translate("حفظ كـ XPS"),
                    MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                LocalizationManager.ShowMessage($"خطأ في حفظ ملف XPS: {ex.Message}", LocalizationManager.Translate("خطأ"),
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
