...
namespace HR_Application
{
    ...
    public partial class ShiftChangeWindow : Window
    {
        ...
        private async void LoadShifts()
        {
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في تحميل الورديات: {ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
            ...
            try
            {
                if (e.Key == Key.Enter)
                {
                    ...
                    if (!string.IsNullOrEmpty(newShift))
                    {
                        ...
                        if (!shiftExists)
                        {
                            LocalizationManager.ShowMessage("هذه الوردية غير موجودة في قاعدة البيانات", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                            return;
                        }
            ...
            try
            {
                ...
                if (attendance != null)
                {
                    ...
                    if (shift != null)
                    {
                        ...
                        LocalizationManager.ShowMessage("تم تغيير الوردية بنجاح", LocalizationManager.Translate("تم"), MessageBoxButton.OK, MessageBoxImage.Information);
                    }
                }
                else
                {
                    LocalizationManager.ShowMessage("لم يتم العثور على سجل الحضور", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                }
            }
            catch (Exception ex)
            {
                LocalizationManager.ShowMessage($"خطأ في تحديث الوردية: {ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
