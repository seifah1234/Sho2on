    ...
    public partial class DataEdit : Window
    {
        ...
        private void DataGet()
        {
            try
            {
                    ...
                    if (userData != null)
                    {
                        ...
                        foreach (var fingerprint in fingerprintData)
                        {
                            ...
                            dataList.Add(new AttendData(
                                ...
                                userData.Id,
                                d.ToString(),
                                (fingerprint.Status == 1) ? LocalizationManager.Translate("حضور") : LocalizationManager.Translate("انصراف"),
                ...
                {
                    LocalizationManager.ShowMessage("ادخل كود الموظف و اختار الفرع");
                }
            }
            catch (Exception ex)
            {
        ...
        private void edit_btn_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                attend.status = (attend.status == LocalizationManager.Translate("انصراف")) ? LocalizationManager.Translate("حضور") : LocalizationManager.Translate("انصراف");
        ...
        private void excel_btn_Click(object sender, RoutedEventArgs e)
        {
                 ...
                 using (var workbook = new XLWorkbook())
            {
                ...
                // Add header row
                worksheet.Cell(1, 1).Value = LocalizationManager.Translate("م");
                worksheet.Cell(1, 2).Value = LocalizationManager.Translate("التاريخ");
                worksheet.Cell(1, 3).Value = LocalizationManager.Translate("الحالة");
                worksheet.Cell(1, 4).Value = LocalizationManager.Translate("الفرع");
                worksheet.Cell(1, 5).Value = LocalizationManager.Translate("الاجراءات");
                worksheet.Cell(1, 6).Value = LocalizationManager.Translate("اليوم");
                worksheet.Cell(1, 7).Value = LocalizationManager.Translate("المستخدم");
        ...
        private void exit_btn_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (selectedItems != null && selectedItems.Count > 0)
            {
                MessageBoxResult result = LocalizationManager.ShowMessage(
                    ...
                    LocalizationManager.Translate("تأكيد"),
        ...
        private void DeleteDuplicate()
        {
            try
            {
                ...
                _context.SaveChanges();
                DataGet();
                LocalizationManager.ShowMessage("تم حذف المكرر");
            }
            catch (Exception ex)
            {
                LocalizationManager.ShowMessage(ex.Message);
        ...
        private void UpdateRows(int rowNum)
        {
            for (int i = rowNum - 1; i < dataList.Count; ++i)
            {
                ...
                attend.statusNo = attend.statusNo == "1" ? "0" : "1";
                attend.status = attend.status == LocalizationManager.Translate("حضور") ? LocalizationManager.Translate("انصراف") : LocalizationManager.Translate("حضور");
            }
            list.ItemsSource = null;
            list.ItemsSource = dataList;
            list.Items.Refresh();
            ...
            try
            {
                if (!string.IsNullOrEmpty(code_box.Text))
                {
                    ...
                    if (user != null)
                    {
                        ...
                        string status = (statusNo == "1") ? LocalizationManager.Translate("حضور") : LocalizationManager.Translate("انصراف");
                        ...
                        LocalizationManager.ShowMessage("تم إضافة البصمة بنجاح");
                        list.Visibility = Visibility.Visible;
                        add_print_grid.Visibility = Visibility.Collapsed;
                    }
                    else
                ...
                {
                    LocalizationManager.ShowMessage("ادخل كود الموظف");
                }
            }
            catch (Exception ex)
            {
        ...
        {
            ConfirmMessage msgBox = new ConfirmMessage(LocalizationManager.Translate("هل أنت متأكد من مراجعة البيانات قبل المعالجة ؟"), LocalizationManager.Translate("متأكد"), LocalizationManager.Translate("لا"));
            msgBox.ShowDialog();
                ...
                if (Keyboard.IsKeyDown(Key.F2) || e.Key == Key.F2)
                {
                    ...
                    if (selectedIndex >= 0)
                    {
                        ...
                        dataList.Insert(insertIndex, new AttendData(
                            ...
                            (newFingerprint.Status == 1) ? LocalizationManager.Translate("حضور") : LocalizationManager.Translate("انصراف"),
        ...
        public async Task InsertAttendanceDataAsync()
        {
            try
            {
                ...
                progressDialog.UpdateStatus(LocalizationManager.Translate("جاري سحب الحركات..."));
                ...
                {
                    LocalizationManager.ShowMessage($"تم إدخال {recordsInserted} سجل جديد", LocalizationManager.Translate("معلومة"), MessageBoxButton.OK, MessageBoxImage.Information);
            ...
            catch (Exception ex)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    LocalizationManager.ShowMessage($"خطأ في إدخال البيانات: {ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                });
            }
        }
        ...
        public async Task InsertData()
        {
            try
            {
                ConfirmMessage msgBox = new ConfirmMessage(LocalizationManager.Translate("هل تريد سحب البيانات ؟"), LocalizationManager.Translate("متأكد"), LocalizationManager.Translate("لا"));
                msgBox.ShowDialog();
                bool result = msgBox.Result;
