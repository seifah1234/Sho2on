    ...
    public partial class DataEdit : Window
    {
        ...
        private void DataGet()
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("«œŒ· ﬂÊœ «·„ÊŸ› Ê «Œ «— «·›—⁄");
                }
            }
            catch (Exception ex)
            {
        ...
        private void edit_btn_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                attend.status = (attend.status == "«‰’—«›") ? "Õ÷Ê—" : "«‰’—«›";
        ...
        private void excel_btn_Click(object sender, RoutedEventArgs e)
        {
                 ...
                 using (var workbook = new XLWorkbook())
            {
                ...
                worksheet.Cell(1, 2).Value = "«· «—ÌŒ";
                worksheet.Cell(1, 3).Value = "«·Õ«·…";
                worksheet.Cell(1, 4).Value = "«·›—⁄";
                worksheet.Cell(1, 5).Value = "«·«Ã—«¡« ";
                worksheet.Cell(1, 6).Value = "«·ÌÊ„";
                worksheet.Cell(1, 7).Value = "«·„” Œœ„";
        ...
        private void DeleteDuplicate()
        {
            try
            {
                ...
                _context.SaveChanges();
                DataGet();
                LocalizationManager.ShowMessage(" „ Õ–› «·„ﬂ——");
            }
            catch (Exception ex)
            {
                LocalizationManager.ShowMessage(ex.Message);
        ...
        private void UpdateRows(int rowNum)
        {
            for (int i = rowNum - 1; i < dataList.Count; ++i)
            {
                ...
                attend.statusNo = attend.statusNo == "1" ? "0" : "1";
                attend.status = attend.status == "Õ÷Ê—" ? "«‰’—«›" : "Õ÷Ê—";
            }
            list.ItemsSource = null;
            list.ItemsSource = dataList;
            list.Items.Refresh();
            ...
            try
            {
                if (!string.IsNullOrEmpty(code_box.Text))
                {
                    ...
                    if (user != null)
                    {
                        ...
                        LocalizationManager.ShowMessage(" „ ≈÷«›… «·»’„… »‰Ã«Õ");
                        list.Visibility = Visibility.Visible;
                        add_print_grid.Visibility = Visibility.Collapsed;
                    }
                    else
                ...
                {
                    LocalizationManager.ShowMessage("«œŒ· ﬂÊœ «·„ÊŸ›");
                }
            }
            catch (Exception ex)
            {
        ...
        {
            ConfirmMessage msgBox = new ConfirmMessage("Â· √‰  „ √ﬂœ „‰ „—«Ã⁄… «·»Ì«‰«  ﬁ»· «·„⁄«·Ã… ø", "„ √ﬂœ", "·«");
            msgBox.ShowDialog();
        ...
        public async Task InsertAttendanceDataAsync()
        {
            try
            {
                ...
                progressDialog.UpdateStatus("Ã«—Ì ”Õ» «·Õ—ﬂ« ...");


                var context = new AppDbContext(App.ConnectionString);
                // «·Õ’Ê· ⁄·Ï «·»Ì«‰«  „‰ «·ÃœÊ· «·„ƒﬁ  machineData
                ...
                {
                    LocalizationManager.ShowMessage($" „ ≈œŒ«· {recordsInserted} ”Ã· ÃœÌœ", "„⁄·Ê„…", MessageBoxButton.OK, MessageBoxImage.Information);
            ...
            catch (Exception ex)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    LocalizationManager.ShowMessage($"Œÿ√ ›Ì ≈œŒ«· «·»Ì«‰« : {ex.Message}", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                });
            }
        }
        ...
        public async Task InsertData()
        {
            try
            {
                ConfirmMessage msgBox = new ConfirmMessage("Â·  —Ìœ ”Õ» «·»Ì«‰«  ø", "„ √ﬂœ", "·«");
                msgBox.ShowDialog();
                bool result = msgBox.Result;
