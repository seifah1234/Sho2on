...
namespace HR_Application.Views.Employees
{
    public partial class PermissionRequestWindow : Window
    {
        ...
        private async void LoadEmployeeByCode(string employeeCode)
        {
            ...
            {
                LocalizationManager.ShowMessage($"Œÿ√ ›Ì  Õ„Ì· »Ì«‰«  «·„ÊŸ›: {ex.Message}", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void SetSelectedEmployee(User employee)
        {
            ...
            // ⁄—÷ „⁄·Ê„«  «·„ÊŸ›
            txtDepartment.Text = employee.Department?.Name ?? "€Ì— „Õœœ";
            txtBranch.Text = employee.Branch?.Name ?? "€Ì— „Õœœ";
            txtJobTitle.Text = employee.JobTitle?.Name ?? "€Ì— „Õœœ";
            txtShift.Text = employee.Shift?.Name ?? "€Ì— „Õœœ";
        ...
        private void btnSelectApprover_Click(object sender, RoutedEventArgs e)
        {
            ...
            var managerSelectionWindow = new EmployeeSelectionWindow(_managers, true, "«Œ — «·„Ê«›ﬁ ⁄·Ï «·≈–‰");
        ...
        private void CalculateDuration()
        {
            ...
            {
                txtDuration.Text = "0";
                LocalizationManager.ShowMessage("Êﬁ  «·‰Â«Ì… ÌÃ» √‰ ÌﬂÊ‰ »⁄œ Êﬁ  «·»œ«Ì…", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
        ...
        private async void btnSubmit_Click(object sender, RoutedEventArgs e)
        {
            if (ValidateForm())
            {
                try
                {
                    ...
                    LocalizationManager.ShowMessage(" „  ﬁœÌ„ ÿ·» «·≈–‰ »‰Ã«Õ ÊÂÊ ﬁÌœ «‰ Ÿ«— «·„Ê«›ﬁ…", "‰Ã«Õ", MessageBoxButton.OK, MessageBoxImage.Information);
                    this.Close();
                }
                catch (Exception ex)
                {
                    LocalizationManager.ShowMessage($"Œÿ√ ›Ì  ﬁœÌ„ «·ÿ·»: {ex.Message}", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
        ...
        {
            var result = LocalizationManager.ShowMessage("Â·  —Ìœ ≈·€«¡ ÿ·» «·≈–‰ø", " √ﬂÌœ «·≈·€«¡",
        ...
        private bool ValidateForm(bool skipManager = false)
        {
            ...
            {
                LocalizationManager.ShowMessage("Ì—ÃÏ «Œ Ì«— «·„ÊŸ› √Ê·«", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
            ...
            {
                LocalizationManager.ShowMessage("Ì—ÃÏ «Œ Ì«— ‰Ê⁄ «·≈–‰", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
            ...
            {
                LocalizationManager.ShowMessage("Ì—ÃÏ  ÕœÌœ  «—ÌŒ «·≈–‰", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
            ...
            if (!txtStartTime.SelectedTime.HasValue || !txtEndTime.SelectedTime.HasValue)
            {
                LocalizationManager.ShowMessage("Ì—ÃÏ ≈œŒ«· Êﬁ  ’ÕÌÕ (HH:mm)", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
            ...
            {
                LocalizationManager.ShowMessage("Ì—ÃÏ «· Õﬁﬁ „‰ «·„œ… «·“„‰Ì… ··≈–‰", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
            ...
            {
                LocalizationManager.ShowMessage("Ì—ÃÏ «Œ Ì«— «·„Ê«›ﬁ ⁄·Ï «·≈–‰", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
            ...
            {
                LocalizationManager.ShowMessage("Ì—ÃÏ ﬂ «»… ”»» «·≈–‰", "Œÿ√", MessageBoxButton.OK, MessageBoxImage.Error);
                txtReason.Focus();
                return false;
            }
