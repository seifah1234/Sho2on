...
namespace HR_Application.Views.Conversations
{
    public partial class ChatWindow : Window, INotifyPropertyChanged
    {
                ...
                foreach (var chat in chats)
                {
                    ...
                    ArchivedChatList.Add(new ChatItemData
                    {
                        ...
                        LastMessage = string.IsNullOrEmpty(lastMsg?.Message)
                                          ? LocalizationManager.Translate("?? مرفق")
                                          : lastMsg?.Message ?? LocalizationManager.Translate("لا توجد رسائل"),
                               ...
                               string senderName)
        {
            ...
            if (groupItem != null)
            {
                groupItem.LastMessage = string.IsNullOrEmpty(message) ? LocalizationManager.Translate("?? مرفق") : message;
            ...
            if (!groupIsOpen)
            {
                ...
                var displayName = !string.IsNullOrEmpty(senderName)
                    ? senderName
                    : (await ctx.Users.FindAsync(senderId))?.FullName ?? LocalizationManager.Translate("مستخدم");

                var shortMsg = string.IsNullOrEmpty(message) ? LocalizationManager.Translate("?? مرفق")
        ...
        private void ChangeBackground_Click(object sender, RoutedEventArgs e)
        {
            var dialog = new Microsoft.Win32.OpenFileDialog
            {
                ...
                Title = LocalizationManager.Translate("اختر خلفية للشات")
            };
        ...
        private void ChatBoxControl_NewMessageReceived(object sender, NewMessageEventArgs e)
        {
            Dispatcher.Invoke(async () =>
            {
                ...
                if (chat != null)
                {
                    chat.LastMessage = string.IsNullOrEmpty(e.Message) ? LocalizationManager.Translate("?? مرفق") : e.Message;
                ...
                if (!chatIsOpen)
                {
                    ...
                    var userName = chatItem?.UserName ?? LocalizationManager.Translate("مستخدم");
        ...
        private async Task LoadChatsAsync()
        {
            try
            {
                ...
                foreach (var chat in chats)
                {
                    ...
                    if (lastMessage == null)
                        lastMessageText = LocalizationManager.Translate("لا توجد رسائل بعد");
                    else if (string.IsNullOrEmpty(lastMessage.Message))
                        lastMessageText = LocalizationManager.Translate("?? مرفق");
                    else
        ...
        private async void AddChatButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                LocalizationManager.ShowMessage("يرجى اختيار مستخدم أولاً", LocalizationManager.Translate("تنبيه"),
            ...
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("المحادثة موجودة بالفعل", LocalizationManager.Translate("معلومات"),
                    ...
                    var chatItem = new ChatItemData
                    {
                        ...
                        LastMessage = LocalizationManager.Translate("ابدأ المحادثة الآن"),
                ...
                else
                {
                    ...
                    ChatList.Add(new ChatItemData
                    {
                        ...
                        UserCode = selectedUser.UserCode,
                        UserId = selectedUser.UserId,
                        LastMessage = LocalizationManager.Translate("محادثة جديدة"),
                        ProfileImageData = selectedUser.ProfileImageData
                    });

                    LocalizationManager.ShowMessage("تم إنشاء المحادثة بنجاح", LocalizationManager.Translate("تم"),
                        MessageBoxButton.OK, MessageBoxImage.Information);
                }
            ...
            {
                LocalizationManager.ShowMessage($"خطأ في إنشاء المحادثة: {ex.Message}", LocalizationManager.Translate("خطأ"),
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
