...
namespace HR_Application.Views
{
    public partial class LoanRequestWindow : Window
    {
        ...
        private async void BtnSearch_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                LocalizationManager.ShowMessage("الرجاء إدخال كود الموظف", LocalizationManager.Translate("تنبيه"), MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            ...
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("الموظف غير موجود", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
                ...
                // حالة الموظف
                txtEmployeeStatus.Text = _currentUser.CanTakeLoan ? LocalizationManager.Translate("مسموح بالسلفة") : LocalizationManager.Translate("غير مسموح بالسلفة");
            }
            catch (Exception ex)
            {
        ...
        private void CalculateLoanDetails()
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage($"مبلغ السلفة يتجاوز الحد المسموح ({maxAllowed:N2})", LocalizationManager.Translate("تنبيه"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                if (basicSalary != null)
                {
                    ...
                    {
                        LocalizationManager.ShowMessage($"القسط الشهري ({monthlyInstallment:N2}) يتجاوز 30% من الراتب ({maxMonthlyInstallment:N2})",
                            LocalizationManager.Translate("تنبيه"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    }
                }
            }
            catch (Exception ex)
            {
                LocalizationManager.ShowMessage($"خطأ في الحساب: {ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async void BtnRequest_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (_currentUser == null)
                {
                    LocalizationManager.ShowMessage("الرجاء البحث عن موظف أولاً", LocalizationManager.Translate("تنبيه"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                if (!_currentUser.CanTakeLoan)
                {
                    LocalizationManager.ShowMessage("هذا الموظف غير مسموح له بأخذ سلفة", LocalizationManager.Translate("تنبيه"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                if (!decimal.TryParse(txtLoanAmount.Text, out decimal loanAmount) || loanAmount <= 0)
                {
                    LocalizationManager.ShowMessage("الرجاء إدخال مبلغ سلفة صحيح", LocalizationManager.Translate("تنبيه"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                if (cmbManagers.SelectedItem == null)
                {
                    LocalizationManager.ShowMessage("الرجاء اختيار مدير للموافقة على السلفة", LocalizationManager.Translate("تنبيه"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                if (string.IsNullOrWhiteSpace(txtReason.Text))
                {
                    LocalizationManager.ShowMessage("الرجاء إدخال سبب السلفة", LocalizationManager.Translate("تنبيه"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }

                if (dpLoanDate.SelectedDate == null || dpExpectedPayback.SelectedDate == null)
                {
                    LocalizationManager.ShowMessage("الرجاء تحديد تاريخ الطلب والتاريخ المتوقع للسداد", LocalizationManager.Translate("تنبيه"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                if (!await friendshipBoxService.CanWithdrawAsync(loanAmount))
                {
                    ...
                    LocalizationManager.ShowMessage($"رصيد صندوق الزمالة غير كافي. الرصيد المتاح: {balance:N2}", LocalizationManager.Translate("تنبيه"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                LocalizationManager.ShowMessage($"تم إرسال طلب السلفة بنجاح للمدير: {selectedManager.FullName}\nبانتظار الموافقة",
