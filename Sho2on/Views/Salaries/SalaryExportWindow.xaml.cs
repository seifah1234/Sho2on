...
namespace HR_Application.Views
{
    public partial class SalaryExportWindow : Window
    {
        ...
        {
            Title = "تصدير بيانات المرتبات إلى Excel";
            Width = 500;
            Height = 300;
            ...
            {
                Content = "ملخص (الحقول الأساسية فقط)",
                Margin = new Thickness(5),
                FontSize = 12
            };
            ...
            {
                Content = "استخدام الأرقام العربية",
                IsChecked = true,
                Margin = new Thickness(5),
                FontSize = 12
            };
        ...
        private void ExportToExcel(bool isDetailed, bool includeHeader, bool autoFormat, bool arabicNumbers)
        {
            try
            {
                ...
                if (saveFileDialog.ShowDialog() == true)
                {
                    ...
                    {
                        var worksheet = workbook.Worksheets.Add("مرتبات");
                        ...
                        if (includeHeader)
                        {
                            ...
                            if (isDetailed)
                            {
                                headers = new[]
                                {
                                    "الكود", "الاسم", "الفرع", "الراتب الأساسي", "الإضافات",
                                    "الاستقطاعات", "صندوق الزمالة", "السلف", "الغياب", "التأخير",
                                    "المشاركة الاجتماعية", "الصافي"
                                };
                            }
                            else
                            {
                                headers = new[]
                                {
                                    "الكود", "الاسم", "الفرع", "الراتب الأساسي",
                                    "الإضافات", "الاستقطاعات", "صندوق الزمالة", "الصافي"
                                };
                            }
                        ...
                        // الإجماليات
                        currentRow++;
                        worksheet.Cell(currentRow, 1).Value = "الإجماليات:";
                        worksheet.Cell(currentRow, 1).Style.Font.Bold = true;
                        ...
                        statsSheet.Cell(1, 1).Value = "إحصائيات المرتبات";
                        ...
                        {
                            { "عدد الموظفين", _employees.Count.ToString() },
                            { "إجمالي الرواتب الأساسية", _employees.Sum(e => e.BasicSalary).ToString("N2") },
                            { "إجمالي الإضافات", _employees.Sum(e => e.Additions).ToString("N2") },
                            { "إجمالي الاستقطاعات", _employees.Sum(e => e.Deductions).ToString("N2") },
                            { "إجمالي صندوق الزمالة", _employees.Sum(e => e.FriendshipBoxAmount).ToString("N2") },
                            { "إجمالي السلف", _employees.Sum(e => e.LoanDeduction).ToString("N2") },
                            { "إجمالي الصافي", _employees.Sum(e => e.NetSalary).ToString("N2") },
                            { "متوسط الراتب", (_employees.Sum(e => e.NetSalary) / _employees.Count).ToString("N2") },
                            { "أعلى راتب", _employees.Max(e => e.NetSalary).ToString("N2") },
                            { "أقل راتب", _employees.Min(e => e.NetSalary).ToString("N2") }
                        };
