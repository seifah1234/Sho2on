    ...
    public partial class DataLoad : Window
    {
        ...
        private async Task LoadDataWithParameters(bool useDateRange)
        {
            try
            {
                ...
                {
                    LocalizationManager.ShowMessage("يرجى اختيار فرع أولاً", LocalizationManager.Translate("تحذير"), MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                if (useDateRange)
                {
                    ...
                    {
                        LocalizationManager.ShowMessage("يرجى تحديد تاريخي البدء", LocalizationManager.Translate("تحذير"),
                ...
                await Dispatcher.InvokeAsync(() =>
                {
                    ...
                    {
                        LocalizationManager.ShowMessage(result, LocalizationManager.Translate("نتيجة التحميل"),
                            MessageBoxButton.OK, MessageBoxImage.Information);
                    }

                    LocalizationManager.ShowMessage("الرجاء انتظار معالجة البيانات...", LocalizationManager.Translate("جاري المعالجة"),
                        MessageBoxButton.OK, MessageBoxImage.Information);
                });
                ...
                await Dispatcher.InvokeAsync(() =>
                {
                    LocalizationManager.ShowMessage("تم الانتهاء من معالجة البيانات", LocalizationManager.Translate("نجاح"),
                        MessageBoxButton.OK, MessageBoxImage.Information);
                });
            }
            catch (Exception ex)
            {
                ...
                {
                    HideLoadingIndicator();
                    LocalizationManager.ShowMessage($"خطأ: {ex.Message}", LocalizationManager.Translate("خطأ"),
        ...
        public void ClearOldData(int branchCode)
        {
            try
            {
                ...
                if (oldRecords.Any())
                {
                    ...
                    {
                        LocalizationManager.ShowMessage($"تم حذف {oldRecords.Count} سجل قديم", LocalizationManager.Translate("معلومة"), MessageBoxButton.OK, MessageBoxImage.Information);
                    });
                }
            }
            catch (Exception ex)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    LocalizationManager.ShowMessage($"خطأ في حذف البيانات القديمة: {ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                });
            }
        }
        ...
        public async Task InsertAttendanceDataAsync(int branchCode)
        {
            try
            {
                ...
                foreach (var machineData in machineDataList)
                {
                    ...
                    catch (Exception ex)
                    {
                        Application.Current.Dispatcher.Invoke(() =>
                        {
                            LocalizationManager.ShowMessage($"خطأ في معالجة سجل للمستخدم {machineData.UserID}: {ex.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Warning);
                        });
                        continue;
                    }
                }
                ...
                Application.Current.Dispatcher.Invoke(() =>
                {
                    ...
                    LocalizationManager.ShowMessage(message, LocalizationManager.Translate("نتيجة الإدخال"), MessageBoxButton.OK, MessageBoxImage.Information);
                });
            ...
            catch (Exception ex)
            {
                Application.Current.Dispatcher.Invoke(() =>
                {
                    LocalizationManager.ShowMessage($"خطأ في إدخال البيانات: {ex.InnerException.Message}", LocalizationManager.Translate("خطأ"), MessageBoxButton.OK, MessageBoxImage.Error);
                });
            }
        }
